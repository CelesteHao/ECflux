# ref: LjZ github: https://github.com/Super-LeoJayZhang/EC_DataFormating_PACCAR/blob/main/src/applyFormating.py
...
group_name = 'GroupC'
data_in = 'C:/Research_function/Hn1/2018/' #the files are Hn1 2018 data in txt
dat_out_nc = data_in + 'Hn1_2018_nc/'
files = glob.glob(os.path.join(data_in, '*.txt'))
# dates = pd.date_range('2020-12-17', '2022-12-15')       # modify based on the dat_in
dates = pd.date_range('2018-01-01', '2019-01-01')  # modify based on the dat_in

# if group_name == 'GroupA':
#     levels = np.array([9, 10, 11, 12])
#     heights = np.array([30.3, 40.2, 50.6, 60.5])
#     sonic_azimuths = np.array([338.8, 337.2, 337.5, 340.3])
# elif group_name == 'GroupB':
#     levels = np.array([5, 6, 7, 8])
#     heights = np.array([8.2, 12.8, 15.8, 23])
#     sonic_azimuths = np.array([336.6, 337.9, 339.6, 339.6])
# elif group_name == 'GroupC':
#     levels = np.array([1, 2, 3, 4])
#     heights = np.array([1.2, 2, 3.5, 6])
#     sonic_azimuths = np.array([291.9, 298.2, 299.1, 300.8])
# else:
...
for date in dates:
    ...
    # Create variables
    base_time = ncfile.createVariable('base_time', 'i8', ('base_time'))
    base_time.unit = 'second since 1970-1-1 0:00:00'
    base_time.aucillary_varuavkes = 'time_offset';
    ...
    time_offset[:] = np.arange(0, 86400, 0.1) + 0.1  # time series data start from 00:00:00.1
    # time_var[:] = np.arange(0, 86400, 0.1)+0.1 # time series data start from 00:00:00.1
    time_var[:] = nc.date2num(df_day.index.to_pydatetime(), units=time_var.units, calendar=time_var.calendar)
    level_var[:] = levels
    height_var[:] = heights
    sonic_azimuth[:] = sonic_azimuths
    ...
    ncfile.setncattr('description', f'multi-layer 10Hz data from EC ({group_name}), contact: zhiheng.lan@wsu.edu')
    ncfile.setncattr('layer height (m)', f'{heights}')
    ncfile.setncattr('Sonic azimuth (degree)', f'{sonic_azimuths}')
    ncfile.close()
